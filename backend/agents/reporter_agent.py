...
def reporter_agent(state: AgentState) -> AgentState:
    ...
    try:
        ...
        slack_message = (
            f"{emoji} *SentinelOps resolved `{incident_id}`*\n"
            f">*Service*: `{service}` | *Severity*: `{severity.upper()}`\n"
            f">*Root Cause*: {root_cause}\n"
            f">*Action*: _{action}_ (confidence {confidence:.0%})"
        )
        ...
        # Update sent status in the local dict we return
        agent_outputs["reporter_agent"]["report_sent"] = report_sent
